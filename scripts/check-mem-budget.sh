...
# if the total is over 7 GiB (ADR-024) or any service has no mem_limit.
#
...
if [ "$total" -gt "$budget" ]; then echo "FAIL: total is over 7 GiB" >&2; status=1; fi
[ "$status" -eq 0 ] && echo "OK: within 7 GiB, every service capped"
exit "$status"
